...
这一步完成后，才能回答“新构造方式是否适配 caption 任务，以及 QA 是否相对旧流程提升”。当前本地机器只有 4GB GTX 1050Ti，且没有 `/cluster` 挂载或可用 `ssh a100` / `ssh 3090`，所以不能在本机完成训练。
...
| remote launcher | `scripts/remote/run_natural_qcc_expansion_smoke_a100.sh`，支持 `PROFILE=a100` 和 `PROFILE=3090` |
...
```bash
cd /cluster/home/hulining/LTSGEN
git fetch origin
git checkout codex/question-repair-20260519-ready
git pull --ff-only origin codex/question-repair-20260519-ready
PROFILE=3090 bash scripts/remote/run_natural_qcc_expansion_smoke_a100.sh
```
...
- `ROOT=/cluster/home/hulining/LTSGEN`
- `PY=/cluster/home/hulining/anaconda3/envs/opentslm/bin/python3`
- `MODEL=Qwen/Qwen3-4B`
- `BRIDGE_TYPE=prefix`
- `NUM_TRAIN_EPOCHS=1`
...
| `natural_qcc_gpu_smoke_result_audit.json` / `.md` | 结果审计；remote launcher 在非 dry-run 时会自动生成 |
...
4. `natural_qcc_gpu_smoke_result_audit.json` 中 `audit_pass=true`。
5. 结果报告中明确比较：
