...
# Full-batch UCR train generation.
# If you use the qwen provider, set QWEN_API_KEY (and optionally QWEN_BASE_URL / QWEN_MODEL).
...
run_one () {
  ...
  python -m ts_cap.cli \
    ...
    --input "$INPUT" \
    --ucr_name "$name" \
    --output "${OUTDIR}/ucr_${name}_train.jsonl" \
    --task classification \
    --ucr_split train \
...
run_one GunPointMaleVersusFemale
run_one GunPointOldVersusYoung
# run_one CricketX
run_one CricketY
run_one CricketZ
