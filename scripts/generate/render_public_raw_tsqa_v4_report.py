...
def bilingual_case_lines(row: dict[str, Any], audit: dict[str, Any]) -> list[str]:
    return [
        ...
        *option_lines(row["options_zh"]),
        ...
        "| Field | English | 中文 |",
        "| --- | --- | --- |",
        f"| Context | {row['context_en']} | {row['context_zh']} |",
        f"| Time Axis | {row['time_series']['time_axis']} | {row['time_series']['time_axis_zh']} |",
        f"| Question | {row['question_en']} | {row['question_zh']} |",
        "",
        "**Variables / 变量**",
        "",
        *[f"- `{name}`: {row['variable_descriptions_en'][name]} / {row['variable_descriptions_zh'][name]}" for name in row["time_series"]["columns"]],
        "",
    ]
